...
namespace GGemCo2DSimulation
{
    ...
    /// </summary>
    public class AddressableLoaderSettingsSimulation : MonoBehaviour
    {
        public static AddressableLoaderSettingsSimulation Instance { get; private set; }

        [HideInInspector] public GGemCoSimulationSettings simulationSettings;

        public delegate void DelegateLoadSettings(GGemCoSimulationSettings simulationSettings);
        ...
        public async Task LoadAllSettingsAsync()
        {
            ...
            {
                // 여러 개의 설정을 병렬적으로 로드
                var taskAttackCombo = LoadSettingsAsync<GGemCoSimulationSettings>(ConfigAddressableSettingSimulation.SimulationSettings.Key);

                // 모든 작업이 완료될 때까지 대기
                await Task.WhenAll(taskAttackCombo);

                // 결과 저장
                simulationSettings = taskAttackCombo.Result;

                // 이벤트 호출
                OnLoadSettings?.Invoke(simulationSettings);
            }
            catch (Exception ex)
            {
