...
namespace GGemCo2DSimulation
{
    public class TempTest : MonoBehaviour
    {
        ...
        public void OnClickSleep()
        {
            ...
            // var simulationDirtyTracker = SimulationPackageManager.Instance.simulationDirtyTracker;
            //
            // foreach (var cell in cells)
            // {
            //     int step   = grindInfo.GetIntSafe(cell, ConfigGridInformationKey.KeySeedStep);
            //     if (step == -1) continue;
            //     int itemUid   = grindInfo.GetIntSafe(cell, ConfigGridInformationKey.KeySeedItemUid);
            //     if (itemUid == -1) continue;
            //     int countWater   = grindInfo.GetIntSafe(cell, ConfigGridInformationKey.KeyWetCount);
            //     
            ...
            //
            //     int next = step + 1;
            //     if (GrowthEvaluator.TryFindNextGrowableStep(growthBase, grindInfo, cell,
            //             next, out var nextStep, out var reason))
            //     {
            //         GrowthEvaluator.ApplyStep(growthBase, tilemap, grindInfo, cell, nextStep, simulationDirtyTracker);
            //         // 물주기 초기화 하기
            ...
            SceneGame.Instance.mapManager.LoadMap(mapUid);
        }
    }
}
